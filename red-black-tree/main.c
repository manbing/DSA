...
static struct rbtree_ops {
	...
	struct rbtree_node *	(*insert)(struct rbtree_node *root, int value);
...
static inline struct rbtree_node *p(struct rbtree_node *curr)
{
	struct rbtree_node *retval = NULL;
...
static inline struct rbtree_node *g(struct rbtree_node *curr)
{
	struct rbtree_node *retval = NULL;

	if (curr && p(curr))
		retval = p(curr)->parents;

	return retval;
}

static inline struct rbtree_node *u(struct rbtree_node *curr)
{
	struct rbtree_node *retval = NULL;

	if (curr && g(curr)) {
		if (g(curr)->left == p(curr))
			retval = g(curr)->right;
		else
			retval = g(curr)->left;
	}
...
static bool ___insert(struct rbtree_node *root, int value)
{
	bool retval = false; 

	if (value > root->value) {
		if (!root->right) {
			struct rbtree_node *new = rbtree_ops.new(value);
			root->right = new;
			new->parents = root;
			retval = true;
		} else {
			retval = ___insert(root->right, value);
		}
		...
		if (!root->left) {
			...
			retval = ___insert(root->left, value);
		}
...
static void check_rbtree_properties(struct rbtree_node *root)
{
	if (!p(root)) {
		root->color = BLACK; 
	}
...
static struct rbtree_node *__insert(struct rbtree_node *root, int value)
{
	if (!root) {
		root = rbtree_ops.new(value);
		root->color = BLACK;
	} else {
		___insert(root, value);
	}
...
static void test_mode(struct rbtree_node **root)
{
	...
	for (i = 0; i < size; i++) {
		*root = rbtree_ops.insert(*root, input[i]);
		//printf("[%s:%d] start\n", __func__, __LINE__);
	...
	while (1) {
		...
		switch (input) {
			case INSERT:
				...
				scanf("%d", &data);
				root = rbtree_ops.insert(root, data);
				break;
